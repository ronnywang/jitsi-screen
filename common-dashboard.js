...
var auto_detach_track = function(){
    ...
    for (var team_id in team_rooms) {
        ...
        for (var id in team_room.participants) {
            for (var track of team_room.participants[id]._tracks) {
                for (var container of track.containers) {
                    var win = container.getRootNode().defaultView;
                    if (win === null || 'undefined' === typeof(win) || win._isUnloading) {
                        track.detach(container);
...
var reselected_users = function(){
    ...
    for (var user_id in selected_users) {
        if (user_id.match(/^t-/)) {
            ...
            var team_id = terms[1];
            var uid = terms[2];
            if ('undefined' === typeof(users[team_id])) {
                users[team_id] = [];
            }
            users[team_id].push(uid);
        } else {
            users.root.push(user_id);
        }
    }

    for (var team_id in users) {
        if (team_id == 'root') {
            room.selectParticipants(users.root);
        } else {
            team_rooms[team_id].selectParticipants(users[team_id]);
        }
    }
};
var selected_users = {};
